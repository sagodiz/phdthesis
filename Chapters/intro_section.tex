...
This dissertation addresses the three aforementioned challenges through three thesis points, see \autoref{tbl:thesispoints}. The \textit{first thesis point, \autoref{chapter_2}} focuses on the problem of LLM comparison, outlining the essential evaluation steps and the main pitfalls. The \textit{second thesis point, \autoref{chapter_4}} investigates hyper-parameter usage and overfitted benchmarks, introducing a dataset to show that temperature remains an underexplored factor in mastering LLM-based code generation. Finally, the \textit{third thesis point, \autoref{chapter_3}} evaluates GPT-4 on vulnerability fixing in a real-world context, providing a clearer view of current model capabilities.

\begin{table}[!h]
	\begin{tabular}{|l|p{0.8\linewidth}|c|}
		\hline
		\textbf{\#} & \textbf{Thesis}                                                                                &  \textbf{Chapter} \\ \hline
		1           &  LLM comparison requires a detailed methodology.                                               &  3                \\ \hline
		2           &  Unpopular datasets provide perfect material for LLM hyper-parameter investigation.            &  4                \\ \hline
		3           &  LLMs cannot properly replace human developers in vulnerability fixing on real-world projects. &  5                \\
		\hline
	\end{tabular}
