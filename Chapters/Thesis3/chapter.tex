...
\chapter{\thesispointthreeuf}
\label{chapter_3}

The previous chapters discuss the use of LLMs in software synthesis.
In this chapter, we take it further by exploring how LLMs, specifically GPT-4, can be applied to vulnerability remediation.
Fixing vulnerabilities requires generating new source code, making the task closely related to program synthesis.
However, it introduces an additional constraint: the corrected code must not only be functionally sound but must also preserve the original behavior while eliminating the vulnerability.

This chapter provides an opportunity to assess the reliability of GPT-4 in performing real-world vulnerability fixing.

\section{Introduction}
\label{sec:intro}

Vulnerability fixing is already discussed in many studies, however, there is no or minimal reliable evaluation of the capabilities of such models that would cover real-life systems and fixing actual vulnerabilities.
...
Similarly, Pearce et al.~\cite{codex_as_fix} employed a language model to generate erroneous C code that is used in a fixing task.
...
Xia et al.~\cite{shared_common_data} also used real projects for evaluating automated repair techniques using the Defects4J~\cite{defects4j} and the ManyBugs~\cite{many_bugs} datasets.
...
Our evaluation included objective test cases and human evaluations, which revealed whether developers could use or at least accept such solutions.
...
\vspace*{1cm}
The author’s contributions in the third thesis point consist of \textbf{(i)} discovering prompt engineering techniques, \textbf{(ii)} designing prompt and model evaluation, \textbf{(iii)} participating in the manual source code evaluation, and \textbf{(iv)} participating in the manual textual response evaluation.
