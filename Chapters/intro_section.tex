...
The ideas, figures, tables and results included in this thesis were published in scientific papers (listed at the end of the thesis).
The scientific papers, although contain human related experiments, are mostly focused on the technical problems introduces previously.
The contributions of the author are briefly introduced as follows:

\vspace*{1em}

\noindent
\textbf{Comparing Large Language Models}: Chapter~\ref{chapter_2} covers the results of the first thesis point~\cite{thesispoint_1}, and discusses the problem of comparing LLMs and introduces a new methodology to support model selection.
Although the methodology may appear straightforward, it extends existing studies in this area, as most LLM comparison research focuses on only a single aspect of a model and, therefore fails to provide a comprehensive perspective.
The study also incorporates the role of human interaction with the LLMs under investigation.
Based on the proposed methodology, a case study is presented that evaluates the two major openly available LLMs at the time of the study.

\vspace*{1em}

\noindent
\textbf{Fixing Vulnerabilities in Real-World Software}: Chapter~\ref{chapter_3} presents the results of the second thesis point~\cite{thesispoint_2}, a practical evaluation of GPT-4 on real-world problems.
The study introduces an evaluation methodology with particular emphasis on prompt engineering, which has emerged as a critical skill in modern software engineering and among LLM users in general.
A case study is performed alongside this methodology, and the results are presented and validated using multiple datasets.

\vspace*{1em}

\noindent
\textbf{Deeper Analysis of LLM Temperature}: Chapter~\ref{chapter_4} introduces the third thesis point~\cite{thesispoint_3}, which addresses the problem of hyper-parameter configuration, with a specific focus on the use of the temperature parameter.
The analysis methodology is discussed with particular emphasis on model selection and evaluation task design.
The study introduces a dataset intended for future use and presents an example use case demonstrating that the temperature parameter does not exhibit a consistent or generic pattern; therefore, further investigation is required.
