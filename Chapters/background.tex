...
The reason for applying metrics in such wide range is that assessing software quality is comparatively easier than other methods and can therefore support easier vulnerability discovery.
...
Besides these metrics rules can be created that check if the code contains elements that should be warned or completely removed from the code, for example misusing cryptographic algorithm~\cite{detect_misused_crypto} or incorrect visibility usage~\cite{implement_security_check_visibility} in security relevant context.
