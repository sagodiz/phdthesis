...
In the \textit{first thesis point~\ref{chapter_2}}, the problem of LLM comparison is addressed.
The chapter discusses which crucial steps must be performed and highlights the main pitfalls associated with these steps.
...
In the \textit{second thesis point~\ref{chapter_4}}, the problems of hyper-parameter usage and overfitted benchmarks are investigated.
A dataset is created to demonstrate that the hyper-parameter temperature remains an underexplored factor in fully mastering LLM-based code generation.
The author’s contributions in the second thesis point consist of \textbf{(i)} designing the study from data creation to evaluation, \textbf{(ii)} developing a model selection framework, \textbf{(iii)} developing a model evaluation framework, and \textbf{(iv)} evaluating the data to investigate the effect of temperature.

In the \textit{third and final thesis point~\ref{chapter_3}}, the GPT-4 model is evaluated on the problem of automatic vulnerability fixing in a real-world context, thereby providing a clearer view of current model capabilities.
