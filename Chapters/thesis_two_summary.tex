...
Inferencing LLMs, especially larger ones, requires special hardware that might not be available for everyone who wants to research LLMs.
...
Those that compiled successfully were taken as successful generations, although those that failed the compilations had to be checked.
We manually checked a statistically significant number of those examples and found that there were no mistakes in our framework: only the LLMs failed to generate source code that compiles.
...
Using this JSON file we also presented that the temperature hyperparameter of LLMs does not hide a pattern for source code generation, and this topic requires more research.
