...
This dissertation addresses the three aforementioned challenges through three thesis points. The \textit{first thesis point~\ref{chapter_2}} focuses on the problem of LLM comparison, outlining the essential evaluation steps and the main pitfalls. The \textit{second thesis point~\ref{chapter_4}} investigates hyper-parameter usage and overfitted benchmarks, introducing a dataset to show that temperature remains an underexplored factor in mastering LLM-based code generation. Finally, the \textit{third thesis point~\ref{chapter_3}} evaluates GPT-4 on vulnerability fixing in a real-world context, providing a clearer view of current model capabilities.
