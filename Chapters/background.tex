...
Masked tokens can be imagined like text on paper, where the paper is stained by coffee, rendering some of the words invisible.
The model's task is to figure out what those invisible words could be.
This prediction is based on the previous and following text; therefore, the generation task is not natural for such models, as they only consider the previous text since there is no subsequent text.
...
The latest publicly available model in this lineage is GPT-5.
...
The generation process is generating the next possible word one-by-one.
Generating 4 words requires 4 cycles of the generation process, and in every iteration, the previously generated token is appended to the old text.
...
Automatic program repair aims to ease the developers' tasks by assisting in the correction of faulty code.
...
Leveraging automated systems, e.g., Language Models, to fix these problems~\cite{conversational_bug_fix_1, same_as_us_but_too_much_hint}, developers get more time to do tasks in the creative field.
This automatic fixing is especially critical in the field of vulnerabilities, where a fix might need immediate attention and cannot wait for a developer to understand the error.
...
Although vulnerability detection is inherently challenging~\cite{vuln_det_challenges}, overall software quality frequently offers useful indicators of where such issues may emerge~\cite{metrics_for_vuln_det}.
Further demonstrating that metrics are suitable for this purpose, several machine learning solutions~\cite{ml_vuln_det,mv_vuln_det_2} have been built on metric-based approaches.
The reason for applying metrics in such a wide range is that assessing software quality is comparatively easier than other methods and can therefore support easier vulnerability discovery.
...
Besides these metrics, rules can be created that check if the code contains elements that should be marked with a warning or completely removed from the code, for example, misusing a cryptographic algorithm~\cite{detect_misused_crypto} or incorrect visibility usage~\cite{implement_security_check_visibility} in security relevant context.
...
%With stable LLM-based solutions creating new metrics or rules or even adapting to a newer languages or language versions could be expressed in natural language, therefore, reducing maintenance costs.
